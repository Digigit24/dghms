...
@receiver(post_save, sender='opd.ProcedureBill')
def create_transaction_for_procedure_bill(sender, instance, created, **kwargs):
    """
    Auto-create transaction when Procedure bill payment_status becomes 'paid'.

    Triggered after:
    - Procedure bill is created with payment_status='paid'
    - Procedure bill is updated and payment_status changes to 'paid'
    """
    from apps.payments.models import Transaction, PaymentCategory
    ...
    if instance.payment_status == 'paid' and instance.received_amount > 0:
        ...
        if not existing_transaction:
            # Get or create Procedure/Investigation income category
            category, _ = PaymentCategory.objects.get_or_create(
                tenant_id=instance.tenant_id,
                name='Procedures & Investigations',
                defaults={
                    'category_type': 'income',
                    'description': 'Income from procedures and investigations'
                }
            )
            ...
            Transaction.objects.create(
                ...
                object_id=instance.id,
                user_id=instance.billed_by_id,
                description=f"Procedure Bill Payment: {instance.bill_number} - {instance.visit.patient.full_name if instance.visit and instance.visit.patient else 'Unknown Patient'}"
            )
...
@receiver(post_save, sender='opd.ProcedureBill')
def create_transaction_for_partial_procedure_payment(sender, instance, **kwargs):
    """
    Create additional transaction when partial payment is received for procedure bill.
    """
    from apps.payments.models import Transaction, PaymentCategory
    from django.db.models import Sum
