from rest_framework import serializers


class TenantMixin(serializers.ModelSerializer):
    """
    Mixin to automatically handle tenant_id from request context
    """

    def create(self, validated_data):
        """Override create to automatically set tenant_id from request"""
        request = self.context.get('request')
        if request and hasattr(request, 'tenant_id'):
            validated_data['tenant_id'] = request.tenant_id
        return super().create(validated_data)

    def update(self, instance, validated_data):
        """Override update to ensure tenant_id cannot be changed"""
        # Remove tenant_id from validated_data if present to prevent changes
        validated_data.pop('tenant_id', None)
        return super().update(instance, validated_data)

    class Meta:
        abstract = True


class TenantViewSetMixin:
    """
    Mixin for ViewSets to automatically filter by tenant_id
    """

    def get_queryset(self):
        """Filter queryset by tenant_id from request"""
        queryset = super().get_queryset()
        if hasattr(self.request, 'tenant_id'):
            return queryset.filter(tenant_id=self.request.tenant_id)
        return queryset

    def perform_create(self, serializer):
        """Ensure tenant_id is set when creating objects"""
        if hasattr(self.request, 'tenant_id'):
            serializer.save(tenant_id=self.request.tenant_id)
        else:
            serializer.save()


class PatientAccessMixin:
    """
    Mixin to restrict patients to their own records
    Patients should only see their own data
    """

    def get_queryset(self):
        """Filter queryset based on user type"""
        queryset = super().get_queryset()

        # If user is a patient, restrict to their own records
        if hasattr(self.request, 'is_patient') and self.request.is_patient:
            # For patient profiles, filter by user_id
            if hasattr(queryset.model, 'user_id'):
                queryset = queryset.filter(user_id=self.request.user_id)
            # For other models that link to patient, add custom logic
            # Example: appointments can be filtered by patient relationship
            elif hasattr(queryset.model, 'patient'):
                queryset = queryset.filter(patient__user_id=self.request.user_id)

        return queryset

    def check_patient_access(self, obj):
        """
        Check if a patient user has access to a specific object
        Raises PermissionDenied if patient tries to access other patient's data
        """
        if hasattr(self.request, 'is_patient') and self.request.is_patient:
            # Check if object belongs to the patient
            if hasattr(obj, 'user_id'):
                if str(obj.user_id) != str(self.request.user_id):
                    from rest_framework.exceptions import PermissionDenied
                    raise PermissionDenied("You don't have access to this resource")
            elif hasattr(obj, 'patient'):
                if hasattr(obj.patient, 'user_id') and str(obj.patient.user_id) != str(self.request.user_id):
                    from rest_framework.exceptions import PermissionDenied
                    raise PermissionDenied("You don't have access to this resource")
